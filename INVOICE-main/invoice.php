<?php
	...
	$pdf->SetFont('Arial','B',16);
	$pdf->SetTextColor(32,100,210);
	$pdf->Cell(150,10,iconv("UTF-8", "ISO-8859-1",strtoupper("Food Place")),0,0,'L');

    $pdf->Ln(9);

    $pdf->SetFont('Arial', '', 10);
    $pdf->SetTextColor(39, 39, 51);
    $pdf->Cell(150, 9, "Direccion: $city, $state, $country", 0, 0, 'L');
    $pdf->Ln(5);

    $pdf->Cell(150, 9, "Teléfono: $restaurantPhone", 0, 0, 'L');
    $pdf->Ln(5);

    // Información del cliente
    $pdf->Cell(150, 9, "Cliente: $clientName $clientLastName", 0, 0, 'L');
    $pdf->Ln(5);

    $pdf->Cell(150, 9, "Email: $clientEmail", 0, 0, 'L');
    $pdf->Ln(5);

    $pdf->Cell(150, 9, "Telefono: $clientPhone", 0, 0, 'L');
    $pdf->Ln(10);

    // Detalles de la reserva
    $pdf->Cell(150, 9, "Fecha de la reserva: $reservationDate", 0, 0, 'L');
    $pdf->Ln(5);

    $pdf->Cell(150, 9, "Hora de la reserva: $reservationTime", 0, 0, 'L');
    $pdf->Ln(10);

    // Total
    $pdf->SetFont('Arial', 'B', 12);
    $pdf->Cell(150, 9, "Total a pagar: $$totalCost USD", 0, 0, 'L');
    $pdf->Ln(10);

    // Mostrar PDF en el navegador
    $pdf->Output("I", "Factura_Reserva_$reservationId.pdf", true);
    exit();
	*/